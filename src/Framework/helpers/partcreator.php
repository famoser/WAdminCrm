...
function GetInput($obj, $prop, $display = null, $type = "text", $special = null, $placeholder = null)
{
    ...
    if ($type == "textarea") {
        ...
        foreach ($special as $item) {
            $add = "";
            if (GetValue($item, "Id") == $val)
                $add = " selected";
            $html .= '<option value="' . GetValue($item, "Id") . '"'.$add.'>' . $item->GetIdentification() . '</option>';
        }
        $html .= '</select>';
    } else {
        ...
        $html .= ">";
        if ($type == "checkbox")
            $html .= GetHiddenInput($prop . "CheckboxPlaceholder", true);
    }

    return $html;
}

function GetHiddenInput($key, $value)
{
